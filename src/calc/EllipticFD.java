...
public class EllipticFD {
  ...
  private void computeEllipticFD(){
    ...
    for (int k = 0; k < nFD; k++){
	  ...
	  Analyzer.getResultsTable().addValue("ax"+k, ax[k]);
	  Analyzer.getResultsTable().addValue("ay"+k, ay[k]);
	  Analyzer.getResultsTable().addValue("bx"+k, bx[k]);
	  Analyzer.getResultsTable().addValue("by"+k, by[k]);
    ...
    for (int k = 0; k < nFD; k++){
      ...
      Analyzer.getResultsTable().addValue("efd"+k, efd[k]);
    }//k-loop for efd
